...
**Préversion `1.0.0-beta.8`.** Cette bêta n'est pas signée Authenticode :
...
3. Télécharger `LIGHTFORMCOLOR-Studio-1.0.0-beta.8-x64-setup.exe`.
...
```powershell
Get-FileHash "$env:USERPROFILE\Downloads\LIGHTFORMCOLOR-Studio-1.0.0-beta.8-x64-setup.exe" -Algorithm SHA256
```
